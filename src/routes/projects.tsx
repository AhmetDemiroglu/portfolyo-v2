    ...
    <motion.div 
      ...
      <div className="p-6 flex flex-col flex-grow dark:bg-slate-800/50">
        ...
        <div className="flex flex-wrap gap-2 my-4">
          {project.tags.map(tag => <span key={tag} className="bg-sky-100 text-sky-800 dark:bg-sky-900/50 dark:text-sky-300 text-xs font-semibold px-2.5 py-1 rounded-full">{tag}</span>)}
        </div>
        <p className="text-slate-600 dark:text-slate-400 flex-grow">{project.description}</p>
        <div className="mt-6 flex items-center gap-4">
          {project.liveLink && <a href={project.liveLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-800 bg-slate-200 rounded-md hover:bg-slate-300 dark:text-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 transition"> <FaExternalLinkAlt /> {t('projects_page.view_live_button')}  </a>}
          {project.githubLink && <a href={project.githubLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-300 bg-slate-700 rounded-md hover:bg-slate-600 transition"> <FaGithub /> {t('projects_page.view_github_button')}  </a>}
        </div>
      </div>
    </motion.div>
  )
    ...
    <motion.div 
      ...
      <div className="flex flex-col md:flex-row gap-8 items-center">
        ...
        <div className="md:w-3/5 flex flex-col justify-center">
          ...
          <div className="mt-6 flex items-center gap-4">
            {project.githubLink && <a href={project.githubLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-3 text-base font-medium text-slate-800 bg-slate-200 rounded-md hover:bg-slate-300 dark:text-white dark:bg-slate-700 dark:hover:bg-slate-600 transition"> <FaGithub /> {t('projects_page.view_github_button_long')} </a>}
          </div>
        </div>
      </div>
    </motion.div>
